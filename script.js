document.addEventListener('DOMContentLoaded', function () {
  ...
  if (contactForm) {
    function getFieldError(field) {
      ...
      if (value !== '') {
        ...
        if (
          (type === 'text' || field.tagName.toLowerCase() === 'textarea') &&
          (name.includes('name') || name.includes('message') || name.includes('subject'))
        ) {
          if (value.length < 2) {
    ...
    contactForm.addEventListener('submit', function (e) {
      ...
      if (!isValid) {
        ...
        if (firstError) {
          ...
          const fieldTop =
            firstError.getBoundingClientRect().top + window.pageYOffset - navbarHeight - 20;
  ...
  if (animatableElements.length > 0) {
    ...
    const styleId = 'scroll-animate-styles';
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "סמי טריגוב טכנאי מיקס ומאסטרינג משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
...
(function() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            var payload = {
                websiteId: 'bcdcfe9a-ce93-44e8-b793-5a933f4e185d',
                name: resolvedName,
                email: resolvedEmail,
            ...
            if (result.success) {
                ...
                if (result.thankYouPagePath && result.ticketNumber) {
                    ...
                    if (isPreview && window.ZAPPY_CONFIG) {
                        var wid = window.ZAPPY_CONFIG.websiteId || 'bcdcfe9a-ce93-44e8-b793-5a933f4e185d';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = 'bcdcfe9a-ce93-44e8-b793-5a933f4e185d';
